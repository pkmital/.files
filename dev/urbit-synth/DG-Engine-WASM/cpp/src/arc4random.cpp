...
static void arc4_stir(void) {
  ...
  if (hLib) {
    ...
    (void *, ULONG) = (BOOLEAN(APIENTRY *)(void *, ULONG))GetProcAddress(
        hLib, "SystemFunction036");
    if (pfn) {
  ...
  int mib[2];
  // unix access to /dev/urandom even when urandom is accessable for some weird
  // reason
  mib[0] = CTL_KERN;
  mib[1] = KERN_ARND;
...
static void arc4_stir_if_needed(void) {
  ...
  if (arc4_count <= 0 ||
      !rs_initialized /*|| arc4_stir_pid != pid*/) { // other threads use the
...
void arc4random_stir(void) { arc4_stir(); }
