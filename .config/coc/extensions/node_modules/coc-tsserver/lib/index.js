...
var _WatchProject = class {
  constructor(service) {
    ...
    task.onExit((code) => {
      if (code != 0) {
        import_coc4.window.showMessage(`TSC exit with code ${code}`, "warning");
      }
      this.onStop();
    });
    task.onStdout((lines) => {
    ...
    task.onStderr((lines) => {
      import_coc4.window.showMessage(`TSC error: ` + lines.join("\n"), "error");
    });
  ...
  async getOptions() {
    ...
    if (!tscPath) {
      import_coc4.window.showMessage(`Local & global tsc not found`, "error");
      return;
    }
    const tsconfigPath = import_coc4.workspace.getConfiguration("tsserver").get("tsconfigPath", "tsconfig.json");
    let find = await import_coc4.workspace.findUp([tsconfigPath]);
    if (!find) {
      import_coc4.window.showMessage(`${tsconfigPath} not found!`, "error");
      return;
    }
...
var FileConfigurationManager = class {
  ...
  enableJavascript() {
    const config = import_coc5.workspace.getConfiguration("tsserver");
  ...
  getCompleteOptions(languageId) {
    ...
    const config = import_coc5.workspace.getConfiguration(`${lang}.suggest`);
    return {
  ...
  getPreferences(language2, uri) {
    ...
    const config = import_coc5.workspace.getConfiguration(language2, uri);
    const preferencesConfig = import_coc5.workspace.getConfiguration(`${language2}.preferences`, uri);
