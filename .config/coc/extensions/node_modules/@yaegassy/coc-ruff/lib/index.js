...
var require_windows = __commonJS({
  "node_modules/isexe/windows.js"(exports, module2) {
    ...
    function checkStat(stat3, path5, options) {
    ...
    function isexe(path5, options, cb) {
      fs5.stat(path5, function(er, stat3) {
        cb(er, er ? false : checkStat(stat3, path5, options));
      });
    }
...
var require_coerce = __commonJS({
  "node_modules/semver/functions/coerce.js"(exports, module2) {
    ...
    var coerce = (version2, options) => {
      ...
      return parse6(`${major2}.${minor2}.${patch}${prerelease}${build}`, options);
    };
...
function getPackageVersion(name) {
  const rootDir = import_path.default.resolve(import_path.default.dirname(__filename), "..");
  const filePath = import_path.default.join(rootDir, "pyproject.toml");
  const fileStr = import_fs.default.readFileSync(filePath);
  const data = import_toml.default.parse(fileStr.toString());
  const version2 = data.tool.poetry.dependencies[name];
  return version2;
}
var RUFF_LSP_VERSION = getPackageVersion("ruff-lsp");
var RUFF_SERVER_CMD = "server";
var RUFF_SERVER_REQUIRED_ARGS = ["--preview"];

// src/client.ts
var import_which = __toESM(require_lib());
function createNativeServerClient(command) {
  const settings = import_coc.workspace.getConfiguration("ruff");
  ...
  const client2 = new import_coc.LanguageClient("ruff", "ruff native server", serverOptions, clientOptions);
  return client2;
}
function createLanguageClient(command) {
  const settings = import_coc.workspace.getConfiguration("ruff");
  const newEnv = { ...process.env };
  const serverOptions = {
    command,
  ...
  const client2 = new import_coc.LanguageClient("ruff", "ruff-lsp", serverOptions, clientOptions);
  return client2;
}
function convertFromWorkspaceConfigToInitializationOptions() {
  ...
  const initializationOptions = {
    settings: {
      ...
      ignoreStandardLibrary: settings.get("ignoreStandardLibrary") ?? true,
      interpreter: settings.get("interpreter"),
      importStrategy: settings.get(`importStrategy`) ?? "fromEnvironment",
      run: settings.get(`run`) ?? "onType",
      organizeImports: settings.get("organizeImports") ?? true,
      fixAll: settings.get("fixAll") ?? true,
      codeAction: {
        fixViolation: {
      ...
      lint: {
        enable: settings.get("lint.enable") ?? true,
        run: getLintRunSetting(),
        args: getLintArgsSetting(),
        preview: settings.get("lint.preview"),
        select: settings.get("lint.select"),
        extendSelect: settings.get("lint.extendSelect"),
        ignore: settings.get("lint.ignore")
      },
      format: {
        args: settings.get("format.args"),
        preview: settings.get("format.preview")
      },
      ...
      enableExperimentalFormatter: settings.get("enableExperimentalFormatter") ?? false,
      exclude: settings.get("exclude"),
      lineLength: settings.get("lineLength"),
      configurationPreference: settings.get("configurationPreference") ?? "editorFirst",
      showSyntaxErrors: settings.get("showSyntaxErrors") ?? true,
...
function getLintArgsSetting() {
  const settings = import_coc.workspace.getConfiguration("ruff");
  if (settings.get("lint.args", []).length > 0) {
    return settings.get("lint.args", []);
  } else if (settings.get("args", []).length > 0) {
    return settings.get("args", []);
  }
  return [];
}
function getLintRunSetting() {
  const settings = import_coc.workspace.getConfiguration("ruff");
  const defaultValue = "onType";
  if (settings.get("run") != null) {
    return settings.get("run", defaultValue);
  }
  return settings.get("lint.run", defaultValue);
}
function getInitializationOptions() {
  const initializationOptions = convertFromWorkspaceConfigToInitializationOptions();
  if (import_coc.workspace.getConfiguration("ruff").get("useDetectRuffCommand")) {
    const envRuffCommandPath = import_which.default.sync("ruff", { nothrow: true });
...
function getLanguageClientDisabledFeatures() {
  const r = [];
  if (getConfigDisableDocumentFormatting())
    r.push("documentFormatting");
  if (getConfigDisableHover())
    r.push("hover");
  return r;
}
function getConfigDisableDocumentFormatting() {
  return import_coc.workspace.getConfiguration("ruff").get("disableDocumentFormatting", false);
}
function getConfigDisableHover() {
  return import_coc.workspace.getConfiguration("ruff").get("disableHover", false);
}

// src/commands/builtinInstallServer.ts
...
var Minimatch = class {
  ...
  partsMatch(a, b, emptyGSMatch = false) {
    ...
    while (ai < a.length && bi < b.length) {
      if (a[ai] === b[bi]) {
        result.push(which3 === "b" ? b[bi] : a[ai]);
        ai++;
        bi++;
...
var PathWin32 = class extends PathBase {
   ...
   */
  getRootString(path5) {
    return import_path2.win32.parse(path5).root;
  }
  /**
   * @internal
   */
...
var pathArg = (path5, opt = {}) => {
  ...
  path5 = (0, import_path3.resolve)(path5);
  ...
  if (platform_default === "win32") {
    const badWinChars = /[*|"<>?:]/;
    const { root: root2 } = (0, import_path3.parse)(path5);
...
var readdirSync2 = (path5) => (0, import_fs5.readdirSync)(path5, { withFileTypes: true });
var chmod = (path5, mode) => new Promise((res, rej) => import_fs3.default.chmod(path5, mode, (er, ...d) => er ? rej(er) : res(...d)));
var mkdir = (path5, options) => new Promise((res, rej) => import_fs3.default.mkdir(path5, options, (er, made) => er ? rej(er) : res(made)));
var readdir2 = (path5) => new Promise((res, rej) => import_fs3.default.readdir(path5, { withFileTypes: true }, (er, data) => er ? rej(er) : res(data)));
var rename = (oldPath, newPath) => new Promise((res, rej) => import_fs3.default.rename(oldPath, newPath, (er, ...d) => er ? rej(er) : res(...d)));
var rm = (path5, options) => new Promise((res, rej) => import_fs3.default.rm(path5, options, (er, ...d) => er ? rej(er) : res(...d)));
var rmdir = (path5) => new Promise((res, rej) => import_fs3.default.rmdir(path5, (er, ...d) => er ? rej(er) : res(...d)));
var stat = (path5) => new Promise((res, rej) => import_fs3.default.stat(path5, (er, data) => er ? rej(er) : res(data)));
var lstat2 = (path5) => new Promise((res, rej) => import_fs3.default.lstat(path5, (er, data) => er ? rej(er) : res(data)));
var unlink = (path5) => new Promise((res, rej) => import_fs3.default.unlink(path5, (er, ...d) => er ? rej(er) : res(...d)));
var promises = {
  chmod,
  mkdir,
  readdir: readdir2,
...
var readdirOrError = (path5) => readdir3(path5).catch((er) => er);
...
var rimrafPosix = async (path5, opt) => {
  ...
  try {
    return await rimrafPosixDir(path5, opt, await lstat3(path5));
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOENT")
      return true;
    throw er;
...
var rimrafPosixSync = (path5, opt) => {
  ...
  try {
    return rimrafPosixDirSync(path5, opt, (0, import_fs4.lstatSync)(path5));
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOENT")
      return true;
    throw er;
...
var rimrafPosixDir = async (path5, opt, ent) => {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafPosixDir((0, import_path4.resolve)(path5, ent2.name), opt, ent2)))).reduce((a, b) => a && b, true);
  if (!removedAll) {
    return false;
  }
...
var rimrafPosixDirSync = (path5, opt, ent) => {
  ...
  if (!Array.isArray(entries)) {
    ...
    ignoreENOENTSync(() => (0, import_fs4.unlinkSync)(path5));
    return true;
  }
  let removedAll = true;
  ...
  ignoreENOENTSync(() => (0, import_fs4.rmdirSync)(path5));
  return true;
};
...
var fixEPERMSync = (fn) => (path5) => {
  try {
    ...
    if ((fer == null ? void 0 : fer.code) === "EPERM") {
      try {
        (0, import_fs4.chmodSync)(path5, 438);
      } catch (er2) {
        const fer2 = er2;
        if ((fer2 == null ? void 0 : fer2.code) === "ENOENT") {
          return;
...
var isDirSync = (path5) => {
  try {
    return (0, import_fs4.statSync)(path5).isDirectory();
  } catch (er) {
    return false;
  }
};
var isDir = (path5) => stat2(path5).then((st) => st.isDirectory(), () => false);
var win32DefaultTmp = async (path5) => {
  const { root } = (0, import_path5.parse)(path5);
...
var uniqueFilename = (path5) => `.${(0, import_path6.basename)(path5)}.${Math.random()}`;
...
var unlinkFixEPERMSync = (path5) => {
  try {
    (0, import_fs4.unlinkSync)(path5);
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "EPERM") {
      try {
        return (0, import_fs4.chmodSync)(path5, 438);
      } catch (er2) {
...
var rimrafMoveRemove = async (path5, opt) => {
  ...
  try {
    return await rimrafMoveRemoveDir(path5, opt, await lstat4(path5));
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOENT")
      return true;
    throw er;
...
var rimrafMoveRemoveDir = async (path5, opt, ent) => {
  ...
  if (path5 === opt.tmp && (0, import_path6.parse)(path5).root !== path5) {
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafMoveRemoveDir((0, import_path6.resolve)(path5, ent2.name), opt, ent2)))).reduce((a, b) => a && b, true);
  if (!removedAll) {
    return false;
  }
  if (opt.preserveRoot === false && path5 === (0, import_path6.parse)(path5).root) {
    return false;
  }
  if (opt.filter && !await opt.filter(path5, ent)) {
    return false;
...
var tmpUnlink = async (path5, tmp, rm3) => {
  const tmpFile = (0, import_path6.resolve)(tmp, uniqueFilename(path5));
...
var rimrafMoveRemoveSync = (path5, opt) => {
  ...
  try {
    return rimrafMoveRemoveDirSync(path5, opt, (0, import_fs4.lstatSync)(path5));
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOENT")
      return true;
    throw er;
...
var rimrafMoveRemoveDirSync = (path5, opt, ent) => {
  ...
  let removedAll = true;
  for (const ent2 of entries) {
    const p = (0, import_path6.resolve)(path5, ent2.name);
  ...
  ignoreENOENTSync(() => tmpUnlinkSync(path5, tmp, import_fs4.rmdirSync));
  return true;
};
var tmpUnlinkSync = (path5, tmp, rmSync2) => {
  const tmpFile = (0, import_path6.resolve)(tmp, uniqueFilename(path5));
  (0, import_fs4.renameSync)(path5, tmpFile);
...
var rimrafWindowsDirMoveRemoveFallback = async (path5, opt) => {
  ...
  try {
    return await rimrafWindowsDirRetry(path5, options);
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOTEMPTY") {
      return await rimrafMoveRemove(path5, options);
    }
    throw er;
  }
};
var rimrafWindowsDirMoveRemoveFallbackSync = (path5, opt) => {
  ...
  const { filter: filter2, ...options } = opt;
  try {
    return rimrafWindowsDirRetrySync(path5, options);
  } catch (er) {
    const fer = er;
    if ((fer == null ? void 0 : fer.code) === "ENOTEMPTY") {
      return rimrafMoveRemoveSync(path5, options);
    }
    throw er;
  }
};
...
var rimrafWindows = async (path5, opt) => {
  ...
  try {
    return await rimrafWindowsDir(path5, opt, await lstat5(path5), START);
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOENT")
      return true;
    throw er;
...
var rimrafWindowsSync = (path5, opt) => {
  ...
  try {
    return rimrafWindowsDirSync(path5, opt, (0, import_fs4.lstatSync)(path5), START);
  } catch (er) {
    if ((er == null ? void 0 : er.code) === "ENOENT")
      return true;
    throw er;
...
var rimrafWindowsDir = async (path5, opt, ent, state = START) => {
  var _a2;
  if ((_a2 = opt == null ? void 0 : opt.signal) == null ? void 0 : _a2.aborted) {
    throw opt.signal.reason;
  }
  ...
  const removedAll = (await Promise.all(entries.map((ent2) => rimrafWindowsDir((0, import_path7.resolve)(path5, ent2.name), opt, ent2, s)))).reduce((a, b) => a && b, true);
  if (state === START) {
    return rimrafWindowsDir(path5, opt, ent, FINISH);
  } else if (state === FINISH) {
    if (opt.preserveRoot === false && path5 === (0, import_path7.parse)(path5).root) {
      return false;
    }
    if (!removedAll) {
      return false;
...
var rimrafWindowsDirSync = (path5, opt, ent, state = START) => {
  const entries = ent.isDirectory() ? readdirOrErrorSync(path5) : null;
  if (!Array.isArray(entries)) {
    if (entries) {
      if (entries.code === "ENOENT") {
  ...
  for (const ent2 of entries) {
    const s = state === START ? CHILD : state;
    const p = (0, import_path7.resolve)(path5, ent2.name);
...
var wrap = (fn) => async (path5, opt) => {
  ...
  if (options.glob) {
    path5 = await glob(path5, options.glob);
  }
  if (Array.isArray(path5)) {
    return !!(await Promise.all(path5.map((p) => fn(path_arg_default(p, options), options)))).reduce((a, b) => a && b, true);
  } else {
    return !!await fn(path_arg_default(path5, options), options);
  }
};
var wrapSync = (fn) => (path5, opt) => {
  const options = optArgSync(opt);
  if (options.glob) {
    path5 = globSync(path5, options.glob);
  }
  if (Array.isArray(path5)) {
    return !!path5.map((p) => fn(path_arg_default(p, options), options)).reduce((a, b) => a && b, true);
  } else {
    return !!fn(path_arg_default(path5, options), options);
  }
};
...
var exec = import_util2.default.promisify(import_child_process.default.exec);
async function ruffLspInstall(pythonCommand, context) {
  const pathVenv = import_path8.default.join(context.storagePath, "ruff-lsp", "venv");
  let pathVenvPython = import_path8.default.join(context.storagePath, "ruff-lsp", "venv", "bin", "python");
  if (process.platform === "win32") {
    pathVenvPython = import_path8.default.join(context.storagePath, "ruff-lsp", "venv", "Scripts", "python");
  }
  const statusItem = import_coc2.window.createStatusBarItem(0, { progress: true });
  statusItem.text = `Installing ruff-lsp in progress...`;
  statusItem.show();
  const installCmd = `"${pythonCommand}" -m venv ${pathVenv} && ${pathVenvPython} -m pip install -U pip ruff-lsp==${RUFF_LSP_VERSION}`;
  rimrafSync(pathVenv);
  try {
    import_coc2.window.showInformationMessage(`Installing ruff-lsp in progress...`);
    await exec(installCmd);
    statusItem.hide();
    import_coc2.window.showInformationMessage(`Installation of ruff-lsp is now complete!`);
  } catch (error) {
    statusItem.hide();
    import_coc2.window.showErrorMessage(`Installation of ruff-lsp failed. | ${error}`);
    throw new Error();
  }
}
async function installWrapper(pythonCommand, context) {
  const msg = "Install/Upgrade ruff-lsp?";
  const ret = await import_coc2.window.showPrompt(msg);
  if (ret) {
    try {
...
var exec2 = import_util3.default.promisify(import_child_process2.default.exec);
function getPythonPath() {
  let pythonPath = import_coc3.workspace.getConfiguration("ruff").get("builtin.pythonPath", "");
  if (pythonPath) {
    return pythonPath;
  }
  pythonPath = import_which2.default.sync("python3", { nothrow: true }) || "";
  if (pythonPath) {
    pythonPath = import_fs13.default.realpathSync(pythonPath);
    return pythonPath;
  }
  pythonPath = import_which2.default.sync("python", { nothrow: true }) || "";
  if (pythonPath) {
    pythonPath = import_fs13.default.realpathSync(pythonPath);
    return pythonPath;
  }
...
function getRuffLspPath(context) {
  let ruffLspPath = import_coc3.workspace.getConfiguration("ruff").get("serverPath", "");
  if (!ruffLspPath) {
    ruffLspPath = import_which2.default.sync("ruff-lsp", { nothrow: true }) || "";
    if (!ruffLspPath) {
      if (import_fs13.default.existsSync(import_path9.default.join(context.storagePath, "ruff-lsp", "venv", "Scripts", "ruff-lsp.exe")) || import_fs13.default.existsSync(import_path9.default.join(context.storagePath, "ruff-lsp", "venv", "bin", "ruff-lsp"))) {
        if (process.platform === "win32") {
          ruffLspPath = import_path9.default.join(context.storagePath, "ruff-lsp", "venv", "Scripts", "ruff-lsp.exe");
        } else {
          ruffLspPath = import_path9.default.join(context.storagePath, "ruff-lsp", "venv", "bin", "ruff-lsp");
        }
      }
    }
  }
...
async function getRuffBinaryPath() {
  let rufrBinaryPath = import_coc3.workspace.getConfiguration("ruff").get("nativeBinaryPath", "");
  if (!rufrBinaryPath) {
    rufrBinaryPath = import_which2.default.sync("ruff", { nothrow: true }) || "";
  }
...
function register(context, client2) {
  context.subscriptions.push(
    import_coc4.commands.registerCommand("ruff.builtin.installServer", async () => {
      ...
      if (!client2) {
        client2 = createLanguageClient(ruffLspPath);
        context.subscriptions.push(import_coc4.services.registLanguageClient(client2));
      } else {
        client2.start();
      }
    })
...
// src/commands/executeAutofix.ts
var import_coc5 = require("coc.nvim");

// src/requestTypes.ts
var lc = __toESM(require("coc.nvim"));
var ExecuteCommandRequestType = new lc.RequestType(
...
async function register2(context, client2) {
  ...
  context.subscriptions.push(
    import_coc5.commands.registerCommand("ruff.executeAutofix", async () => {
      if (!client2) {
        return;
      }
      const textEditor = import_coc5.window.activeTextEditor;
      if (!textEditor) {
        return;
      }
      const textDocument = {
      ...
      await client2.sendRequest(ExecuteCommandRequestType, params).then(void 0, async () => {
        await import_coc5.window.showErrorMessage(
...
// src/commands/executeFormat.ts
var import_coc6 = require("coc.nvim");
async function register3(context, client2) {
  await client2.onReady();
  context.subscriptions.push(
    import_coc6.commands.registerCommand("ruff.executeFormat", async () => {
      if (!client2) {
        return;
      }
      const textEditor = import_coc6.window.activeTextEditor;
      if (!textEditor) {
        return;
      }
      const textDocument = {
      ...
      await client2.sendRequest(ExecuteCommandRequestType, params).then(void 0, async () => {
        await import_coc6.window.showErrorMessage(
...
// src/commands/executeOrganizeImports.ts
var import_coc7 = require("coc.nvim");
async function register4(context, client2) {
  await client2.onReady();
  context.subscriptions.push(
    import_coc7.commands.registerCommand("ruff.executeOrganizeImports", async () => {
      if (!client2) {
        return;
      }
      const textEditor = import_coc7.window.activeTextEditor;
      if (!textEditor) {
        return;
      }
      const textDocument = {
      ...
      await client2.sendRequest(ExecuteCommandRequestType, params).then(void 0, async () => {
        await import_coc7.window.showErrorMessage(
          "Failed to apply Ruff fixes to the document. Please consider opening an issue with steps to reproduce."
        );
      });
    })
...
function register5(context, client2) {
  context.subscriptions.push(
    import_coc8.commands.registerCommand("ruff.restart", async () => {
      if (client2) {
        if (client2.serviceState !== import_coc8.ServiceStat.Stopped) {
          await client2.stop();
        }
      }
      client2.start();
...
// src/commands/debugInformation.ts
var import_coc9 = require("coc.nvim");
async function register6(context, client2) {
  await client2.onReady();
  context.subscriptions.push(
    import_coc9.commands.registerCommand("ruff.debugInformation", async () => {
      if (!client2 || !import_coc9.workspace.getConfiguration("ruff").get("nativeServer")) {
        return;
      }
      const params = {
      ...
      await client2.sendRequest(ExecuteCommandRequestType, params).then(void 0, async () => {
        await import_coc9.window.showErrorMessage("Failed to print debug information.");
      });
...
async function register7(context, client2) {
  await client2.onReady();
  context.subscriptions.push(
    import_coc10.commands.registerCommand("ruff.showLogs", () => {
...
// src/features/autoFixOnSave.ts
var import_coc11 = require("coc.nvim");
async function register8(client2) {
  await client2.onReady();
  if (!import_coc11.workspace.getConfiguration("ruff").get("autoFixOnSave", false))
    return;
  import_coc11.workspace.registerAutocmd({
    request: true,
    event: "BufWritePre",
    pattern: "*.py",
    callback: async () => {
      const { document } = await import_coc11.workspace.getCurrentState();
      if (!client2.diagnostics)
        return;
      const currentDiags = client2.diagnostics.get(document.uri);
      if (!currentDiags)
        return;
      let existsFix = false;
      ...
      if (existsFix) {
        await import_coc11.workspace.nvim.call("CocAction", ["runCommand", "ruff.executeAutofix"]);
      }
    }
  });
}
...
async function register9(context, client2) {
  await client2.onReady();
  if (!import_coc12.workspace.getConfiguration("ruff").get("client.codeAction.showDocumantaion.enable", false))
    return;
  const documentSelector = [{ scheme: "file", language: "python" }];
  context.subscriptions.push(
    import_coc12.languages.registerCodeActionProvider(documentSelector, new ShowDocumentationCodeActionProvider(client2), "ruff")
  );
}
var ShowDocumentationCodeActionProvider = class {
  ...
  async provideCodeActions(document, range, context) {
    ...
    const wholeRange = import_coc12.Range.create(0, 0, doc.lineCount, 0);
    let whole = false;
    ...
    if (this.lineRange(range) && context.diagnostics.length > 0) {
      ...
      if (line && line.length) {
        const ruffRuleContents = [];
        context.diagnostics.forEach((d) => {
          var _a2;
          if (d.source === this.source) {
            if ("codeDescription" in d) {
              const ruffDiagnostic = d;
              if ((_a2 = ruffDiagnostic.codeDescription) == null ? void 0 : _a2.href) {
...
async function activate(context) {
  if (!import_coc13.workspace.getConfiguration("ruff").get("enable"))
    return;
  const extensionStoragePath = context.storagePath;
  if (!import_fs14.default.existsSync(extensionStoragePath)) {
    import_fs14.default.mkdirSync(extensionStoragePath, { recursive: true });
  }
  if (import_coc13.workspace.getConfiguration("ruff").get("nativeServer")) {
    const command = await getRuffBinaryPath();
    if (command) {
      client = createNativeServerClient(command);
    } else {
      import_coc13.window.showWarningMessage('coc-ruff | "ruff" binary does not exist.`');
      return;
    }
  } else {
    const ruffLspPath = getRuffLspPath(context);
    if (!ruffLspPath || !import_fs14.default.existsSync(ruffLspPath)) {
      register(context, client);
      import_coc13.window.showWarningMessage(
  ...
  context.subscriptions.push(import_coc13.services.registLanguageClient(client));
  register(context, client);
  register7(context, client);
  register2(context, client);
  register4(context, client);
  register3(context, client);
  register6(context, client);
  register5(context, client);
  register8(client);
  register9(context, client);
}
